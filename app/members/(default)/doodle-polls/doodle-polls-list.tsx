...
export default function DoodlePollsList({
  ...
  const [eventType, setEventType] = useQueryState('type', {
    ...
    parse: (value): 'all' | DoodleEventType => {
      ...
      return validTypes.includes(value as DoodleEventType | 'all') ? (value as any) : 'all';
    },
  });
  
  const [sortBy, setSortBy] = useQueryState<SortBy>('sortBy', {
    defaultValue: 'event_date',
    parse: (value) => (value === 'created_date' ? 'created_date' : 'event_date'),
  });
  ...
  const [hasSyncedFromLocalStorage, setHasSyncedFromLocalStorage] = useState(false);
    ...
    <div className="space-y-6">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <Button
            onClick={() => {
              const newPage = Math.min(totalPages, parseInt(currentPage) + 1).toString();
              setCurrentPage(newPage);
            }}
            disabled={parseInt(currentPage) === totalPages}
            variant="outline"
